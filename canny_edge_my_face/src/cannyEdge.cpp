...
namespace canny_edge_my_face
{
	CannyEdge::CannyEdge(ros::NodeHandle nh) : it(nh) {
		imgSub_ = it.subscribe("/cv_camera/image_raw", 1, &CannyEdge::process, this); //call process with raw image as input
	...
	void CannyEdge::process(const sensor_msgs::ImageConstPtr& srcImg) {
		imgPubRaw_.publish(srcImg); // Publish Raw Image
		cv_bridge::CvImagePtr cv_ptr; 
    		try {
			cv_ptr = cv_bridge::toCvCopy(srcImg, sensor_msgs::image_encodings::BGR8); // Make ROS image into OpenCv Image
		}
		...
		img = img+cv_ptr->image; // Overlay canny filter to original image
		...
		imgPub_.publish(msg); // Publish Original image with canny edge overlay
